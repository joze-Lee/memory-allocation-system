...
CFLAGS = -Wall -std=gnu99 -O3 -pthread -DNDEBUG=1
...
libMallocInstrumented.so: mm.c list.c memlib.c mallocanalysis.c
	...
	LD_LIBRARY_PATH=$(INST_BUILD_LOC)/lib opt -enable-new-pm=0 -load  mm.bc -o mm-instrumented.bc
	llc $(LLCFLAGS) mm-instrumented.bc -o mm-instrumented.o

	# This compiles and instruments list.c
	clang $(CLANGFLAGS) list.c -o list.bc
	LD_LIBRARY_PATH=$(INST_BUILD_LOC)/lib opt -enable-new-pm=0 -load  list.bc -o list-instrumented.bc
